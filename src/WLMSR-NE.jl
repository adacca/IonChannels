...
#edited to have sliding detailed balance violation in inactivation transitions

"""
	Gmatrix(alpha,epsilon)

calculates transition rate matrix for 5-state K+ model, calls param(alpha)
inputs:
alpha--Float64, transmembrane voltage in mV
epsilon--Float64, detailed balance violation tuner [0,1]
returns:
5x5 matrix of transition rates for K+ channel
""" 
function Gmatrix(alpha,epsilon)
    ...
    return [-a1   a1     0        0      0 ;
             ...
             0    Kb   -(Kb+a2)   a2     0 ;
             0    0      b2     -(b2+ai) ai;
             0    0      bi*epsilon        bi*(1-epsilon)    -bi]
    end
"""
	param(alpha)
