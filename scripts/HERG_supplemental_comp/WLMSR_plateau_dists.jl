...
plt1 = plot(legend=:topleft,title="Distance from steady state (WLMSR)",xaxis="time [ms]",yaxis="Occupancy difference [%]",dpi=200)#,size=(400,400),title="B",titlelocation=:left) #full graph
plt1 = plot!(CR["tL"][:,1],dss,xlims=(300,500),color=:black,labels=["C3" "C2" "C1" "O" "I"],linestyles=[:dot :dash :dashdot :dashdotdot :solid])
plt1 = plot!(twinx(),CR["tL"][:,1],CR["tL"][:,2],linestyle=:dash,linecolor=:grey,ylabel="Transmembrane voltage [mV]",label=:none,xlims=(300,500))

display(plt1)
readline()

figname= "WLMSR_plateau_dist.png"
wsave(datadir("figs",figname),plt1)
