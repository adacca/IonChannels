...
#user analysis
using Plots
plt1 = plot(legend=:topleft,xaxis="time [ms]",yaxis="Total entropy production [1/kBT/s]",dpi=200)#,size=(400,400),title="B",titlelocation=:left) #full graph
...
plt1 = plot!(CR["tL"][:,1],CR["Stot"],label="CR",xlims=(300,500),color=:black,linestyle=:dot)
plt1 = plot!(MGWMN["tL"][:,1],MGWMN["Stot"],label="MGWMN",xlims=(300,500),color=:black,linestyle=:dash)
plt1 = plot!(WLMSR["tL"][:,1],WLMSR["Stot"],label="WLMSR",xlims=(300,500),color=:black,linestyle=:dashdotdot)
plt1 = plot!(OGD["tL"][:,1],OGD["Stot"],label="OGD",xlims=(300,500),color=:black,linestyle=:solid)
#display(plt1)
#readline()

#saving data 
figname= "HERG_ap_Stot_detail.png"
