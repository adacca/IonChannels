...
#user analysis
using Plots
plt1 = plot(legend=:topleft,xaxis="time [ms]",yaxis="System entropy production [1/kBT/s]",dpi=200)#,size=(400,400),title="B",titlelocation=:left) #full graph
...
plt1 = plot!(CR["tL"][:,1],CR["Ssys"],label="CR",xlims=(300,500),ylims=(-0.2,0.8),color=:black,linestyle=:dot)
plt1 = plot!(MGWMN["tL"][:,1],MGWMN["Ssys"],label="MGWMN",xlims=(300,500),ylims=(-0.2,0.8),color=:black,linestyle=:dash)
plt1 = plot!(WLMSR["tL"][:,1],WLMSR["Ssys"],label="WLMSR",xlims=(300,500),ylims=(-0.2,0.8),color=:black,linestyle=:dashdotdot)
plt1 = plot!(OGD["tL"][:,1],OGD["Ssys"],label="OGD",xlims=(300,500),ylims=(-0.2,0.8),color=:black,linestyle=:solid)
plt1 = plot!(twinx(),CR["tL"][:,1],CR["tL"][:,2],linestyle=:dash,linecolor=:grey,ylabel="Transmembrane voltage [mV]",label=:none,xlims=(300,500))#,xlims=(65,80) #moved to here so ylims wouldn;t mess w it, change once removed

#display(plt1)
#readline()

#saving data 
figname= "HERG_ap_Ssys.png"
