...
dt=1e-3
using Plots
plt1 = plot(legend=:topleft,xaxis="time [ms]",yaxis="Rate of Shannon Entropy Production")#,size=(400,400),title="B",titlelocation=:left) #full graph
#plt1 = plot(legend=:topleft,xaxis="time [ms]",yaxis="Total entropy production [1/kBT]",size=(400,400),xlims=(65,80),ylims=(-0.01,0.4))
plt1 = plot!(twinx(),CR["tL"][:,1],CR["tL"][:,2],linestyle=:dash,linecolor=:grey,ylabel="Transmembrane voltage [mV]",label=:none)#,xlims=(65,80)
plt1 = plot!(CR["tL"][:,1],IonChannelTools.ddt(CR["H"],dt),label="CR")
plt1 = plot!(MGWMN["tL"][:,1],IonChannelTools.ddt(MGWMN["H"],dt),label="MGWMN")
plt1 = plot!(WLMSR["tL"][:,1],IonChannelTools.ddt(WLMSR["H"],dt),label="WLMSR")
plt1 = plot!(OGD["tL"][:,1],IonChannelTools.ddt(OGD["H"],dt),label="OGD")
#display(plt1)
#readline()
