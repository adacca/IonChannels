...
#user analysis
using Plots
plt1 = plot(legend=:topleft,xaxis="time [ms]",yaxis="External entropy production i think [kBT]")#,size=(400,400),title="B",titlelocation=:left) #full graph
...
plt1 = plot!(CR["tL"][:,1],CR["Smed"],label="CR")
plt1 = plot!(MGWMN["tL"][:,1],MGWMN["Smed"],label="MGWMN")
plt1 = plot!(WLMSR["tL"][:,1],WLMSR["Smed"],label="WLMSR")
plt1 = plot!(OGD["tL"][:,1],OGD["Smed"],label="OGD")
#display(plt1)
#readline()

#saving data 
figname= "HERG_ap_Smed.png"
