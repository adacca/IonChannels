...
plt1 = plot(legend=:bottomright,xaxis="time [ms]",yaxis="Relative Shannon Entropy",dpi=200)#,size=(400,400),title="B",titlelocation=:left) #full graph
...
plt1 = plot!(CR["tL"][:,1],CR["H"]./Hmax,label="CR",xlims=(300,500),color=:black,linestyle=:dot)
plt1 = plot!(MGWMN["tL"][:,1],MGWMN["H"]./Hmax,label="MGWMN",xlims=(300,500),color=:black,linestyle=:dash)
plt1 = plot!(WLMSR["tL"][:,1],WLMSR["H"]./Hmax,label="WLMSR",xlims=(300,500),color=:black,linestyle=:dashdotdot)
plt1 = plot!(OGD["tL"][:,1],OGD["H"]./OGDmax,label="OGD",xlims=(300,500),color=:black,linestyle=:solid)
#display(plt1)
#readline()

#saving data 
figname= "HERG_ap_H_rel.png"
