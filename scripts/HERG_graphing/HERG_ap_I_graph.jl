...
#user analysis
using Plots
plt1 = plot(legend=:topleft,xaxis="time [ms]",yaxis="Average current",dpi=200)#,size=(400,400))
plt1 = plot!(twinx(),CR["tL"][:,1],CR["tL"][:,2],linestyle=:dash,linecolor=:grey,ylabel="Transmembrane voltage [mV]",label=:none)
plt1 = plot!(CR["tL"][:,1],CR["I"],label="CR",color=:black,linestyle=:dot)
plt1 = plot!(MGWMN["tL"][:,1],MGWMN["I"],label="MGWMN",color=:black,linestyle=:dash)
plt1 = plot!(WLMSR["tL"][:,1],WLMSR["I"],label="WLMSR",color=:black,linestyle=:dashdotdot)
plt1 = plot!(OGD["tL"][:,1],OGD["I"],label="OGD",color=:black,linestyle=:solid)
