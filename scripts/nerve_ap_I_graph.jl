...
#user analysis
using Plots
plt1 = plot(legend=:topleft,xaxis="time [ms]",yaxis="Average current (K+)",dpi=200)#,size=(400,400))
#plt1 = plot!(twinx(),CR["tL"][:,1],CR["tL"][:,2],linestyle=:dash,linecolor=:grey,ylabel="Transmembrane voltage [mV]",label=:none)#,xlims=(0,250))
plt1 = plot!(twinx(),Na["tL"][:,1],Na["I"],yaxis=:flip,label="Na+",ylabel="Average current (Na+)",color=:black,linestyle=:solid)
plt1 = plot!(K["tL"][:,1],K["I"],label="K+",color=:black,linestyle=:dashdotdot)
plt1 = plot!(title="",titlelocation=:left) #should be size in mm
plt1 = plot!(twinx(),K["tL"][:,1],K["tL"][:,2],showaxis=false,linestyle=:dash,linecolor=:grey,label=:none)

display(plt1)
#readline()

#saving data 
figname= "nerve_ap_I.png"
