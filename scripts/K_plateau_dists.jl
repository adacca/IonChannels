...
#P1=-20mV
#retrieving data
source_file = "model=K5_prot_name=spike_dt=0.001_Largs=rs_tbound=200.jld2" #name of file to be loaded (path not needed, extension needed)
...
plt1 = plot(legend=:topleft,title="Distance from steady state (K+)",xaxis="time [ms]",yaxis="Occupancy difference [%]")#,size=(400,400),title="B",titlelocation=:left) #full graph
plt1 = plot!(K["tL"][:,1],dss,labels=["C4" "C3" "C2" "C1" "O"])
plt1 = plot!(twinx(),K["tL"][:,1],K["tL"][:,2],linestyle=:dash,linecolor=:grey,ylabel="Transmembrane voltage [mV]",label=:none)

display(plt1)
#readline()

figname= "K_plateau_dist.png"
wsave(datadir("figs",figname),plt1)
